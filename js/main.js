...
const config = {
	...
	colorDistanceThreshold: 100,	// used to make sure foreground colour and background aren't too similar (MAX == distance(white, black) == sqrt(195075) ~ 441.67)
...
const wallpaperMediaThumbnailListener = event => {
	...
	if (config.audioChangeColour) {
		...
		if (config.audioChangeRainColourOnly) {
			if (colorDistance(config.userBackgroundColour, config.songPrimaryColour) < config.colorDistanceThreshold)
				fgcol = colorAlongVector(config.userBackgroundColour, config.songPrimaryColour);
			else
